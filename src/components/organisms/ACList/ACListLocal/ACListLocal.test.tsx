...
import { ACListItem } from "../ACListBase/ACListBase";

import { MdAdd, MdDelete, MdEdit, MdInfo } from "react-icons/md";

const testDataItems: ACListItem[] = [];
for (let i = 1; i <= 7; i++) {
  testDataItems.push({
    id: i.toString(),
    title: "Title " + i,
...
describe("ListLocal", () => {
  it("renders the default ListLocal component with no items", () => {
    render(<ACListLocal headerTitle={"local list header"} allItems={[]} />);
    expect(screen.getByTestId("AC-List")).toBeInTheDocument();
  });

  it("renders ListLocal component with items", () => {
    render(
      <ACListLocal headerTitle={"header title"} allItems={testDataItems} />
    );

    expect(screen.getByTestId("AC-List")).toBeInTheDocument();
    expect(screen.getAllByTestId("AC-List-Items-Item").length === 5);
  });
  ...
  it("tests if sorting items works", () => {
    ...
    const itemsParentSelect = screen.getByTestId("AC-List-Items");
  ...
  it("tests if searching items works", async () => {
    ...
    await waitFor(() =>
      expect(screen.getByTestId("AC-List-Items").children.length).toBe(1)
    );
    ...
    await waitFor(() =>
      expect(screen.getByTestId("AC-List-Items").children.length).toBe(5)
    );
  });
  ...
  it("tests if actions are rendered", () => {
    ...
    expect(screen.getAllByTestId("AC-List-Items-Item-Icons").length).toEqual(5);
  });
  ...
  it("tests if custom item actions are called", async () => {
    ...
    userEvent.click(
      screen.getAllByTestId("AC-List-Items-Item-Icons")[0].children[0]
        .children[0]
    );
